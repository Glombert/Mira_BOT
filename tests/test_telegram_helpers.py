...
class TestSplitMessage:
    ...
    def test_exactly_limit(self):
        s = "a" * bot.config.MAX_MSG_LEN
        assert tb._split_message(s) == [s]

    def test_over_limit_two_parts(self):
        s = "a" * (bot.config.MAX_MSG_LEN + 1)
        parts = tb._split_message(s)
        assert len(parts) == 2
        assert "".join(parts) == s
        assert all(len(p) <= bot.config.MAX_MSG_LEN for p in parts)

    def test_multiple_parts_preserve_content(self):
        s = "x" * (bot.config.MAX_MSG_LEN * 2 + 17)
        parts = tb._split_message(s)
        assert len(parts) == 3
        assert "".join(parts) == s
        assert all(len(p) <= bot.config.MAX_MSG_LEN for p in parts)
...
class TestHelpKeyboard:
    ...
    def test_non_owner_basic_no_admin(self):
        cbs = self._callbacks(bot.menu._help_keyboard(False))
    ...
    def test_owner_has_admin(self):
        cbs = self._callbacks(bot.menu._help_keyboard(True))
        assert {"cmd_reflect", "cmd_rollback", "cmd_release", "cmd_users"} <= set(cbs)

    def test_owner_more_rows(self):
        assert len(bot.menu._help_keyboard(True).inline_keyboard) > len(bot.menu._help_keyboard(False).inline_keyboard)
