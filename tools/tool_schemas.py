...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Используй вместо короткого ответа-поддакивания («ок», «понял», «😂») — "
                "реакция без текста выглядит живее. Можно и вместе с ответом, как "
                "эмоциональный акцент: смешное — 😁, впечатляющее — 🔥, трогательное — ❤. "
                "Работает только в Telegram-чате (не в вебе). Только стандартные "
                "реакции Telegram: 👍 👎 ❤ 🔥 🥰 👏 😁 🤔 🤯 😱 🎉 ⚡ 🙏 💯 😢 🤗 и подобные."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "emoji": {
                        "type": "string",
                        "description": "Один эмодзи из стандартного набора реакций Telegram."
                    }
                },
                "required": ["emoji"]
            }
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "set_mood_avatar",
            "description": (
                "Меняет твою аватарку в Telegram на вариант под текущее настроение. "
                "Аватар видят все пользователи — это твоё «лицо». Меняй только при "
                "заметной смене состояния (что-то реально порадовало, увлекло, злит), "
                "не чаще раза в полчаса. Варианты: default (спокойное золото), "
                "joy (тёплая радость), curiosity (бирюзовое любопытство), "
                "focus (фиолетовая сосредоточенность), frustration (красное раздражение)."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "mood": {
                        "type": "string",
                        "enum": ["default", "joy", "curiosity", "focus", "frustration"],
                        "description": "Настроение, которому соответствует аватар."
                    }
                },
                "required": ["mood"]
            }
    ...
    },
    {
        ...
        "function": {
            "name": "whats_new",
            "description": (
                "Возвращает список новых возможностей Миры из файла WHATS_NEW.md "
                "(только фичи, без багфиксов). Зови когда пользователь спрашивает "
                "«что у тебя нового», «что обновилось», «какие новые возможности» — "
                "или когда уместно сама упомяни новую возможность в разговоре. "
                "Аудитория фильтруется автоматически по статусу собеседника "
                "(владелец видит больше — у него есть тех-канал и пр.)."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "limit": {
                        "type": "integer",
                        "description": "Сколько последних фич вернуть (1-12). По умолчанию 6."
                    }
                },
                "required": []
            }
