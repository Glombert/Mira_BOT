...
> ⚠️ **АРХИВ / ИСТОРИЯ.** Этот документ — dev-лог разработки до **2026-05-12**
> (внутренняя нумерация v3.x). Он НЕ отражает редизайн Aurora, мобильные
> клиенты, продуктовые версии v2.3/v2.4 и инверсию маршрутизации (роутер→Конклав
> убран). Часть описанных механизмов уже изменена.
>
> **Актуальное:** changelog — в `README.md` (раздел «Что нового»); правила
> поведения Миры — в `RULES.md`/`behavior.md`; архитектура — в `ARCHITECTURE.md`.
> Оставлен как историческая справка.
...
## Что изменилось в v3.11

Семантическая память + disaster recovery:

- **ChromaDB для семантической памяти.** `tools/semantic_memory.py` — каждое сообщение (юзера и Миры) индексируется в локальную векторную базу `memory/chroma/`. Перед ответом Мира ищет по смыслу релевантные фрагменты прошлых разговоров и подмешивает в системный промпт ("Из прошлых разговоров вспомнила…"). Эмбеддинги — встроенный ONNX MiniLM-L6-v2 (без API-ключей, ~80MB при первом запуске).
- **Новый инструмент `recall(query)`.** Мира может явно искать по памяти — "помнишь когда мы обсуждали X". Top-K с метаданными (роль, дата, distance).
- **Чистка при `/forget` и удалении пользователя.** Векторы удаляются вместе с профилем.
- **Disaster Recovery v1.0.** `scripts/backup_env.sh` шифрует `.env` через GPG и кладёт в `gdrive:Mira/secrets/env.gpg`. `restore_env.sh` обратный путь. Раздел "Disaster Recovery" в README с пошаговой инструкцией переноса Миры на новый сервер.
- **`versions/` в ежедневном бэкапе** — теперь и история эволюции (бэкапы `agent.py` + `persona.json` перед самоизменениями) едет на Drive.
- **`reflections` вынесены из `persona.json`** в `memory/reflections.json` (git-untracked) — больше никаких конфликтов при `git pull` после самоизменений Миры.
- **Аудит и багфиксы:**
  - `set_status` не был импортирован в `telegram_bot.py` — кнопка "В гости" в карточке пользователя крашилась
  - `cleanup_expired_guests()` не вызывался в telegram-режиме — теперь чистит на старте
  - `scripts/notify.sh` теперь executable в git навсегда
  - Дубликат subprocess.run для smoke-test → теперь оба пути (`/evolve` CLI и Telegram) используют одну `smoke_test()`
  - Мёртвые импорты, неиспользуемые f-string

---

## Что изменилось в v3.10

Полировка UX, живой стиль общения, баг-фиксы:

- **Живой голос в прогрессе Конклава.** Раньше прогресс был одинаковым набором фраз ("Разведчик работает...", "Редактор улучшает результат...", "Критик доволен (10/10)"). Теперь — голос Миры с вариативными фразами через `random.choice`: "Так, копаю в инете", "Сейчас Кодер возьмётся", "Готово, сейчас расскажу". Промежуточные стадии (Editor, Critic) убраны — они слишком внутренние. Стартовое сообщение от бота тоже убрано — первый прогресс от Конклава его заменяет.
- **Персона переработана** (черновик от Gemini + дополнения). Жёсткий `anti_patterns` блок: запрет "Конечно!", "Отличный вопрос!", "Безусловно!". Слова-паразиты ("ну", "слушай", "блин", "кстати") для естественной речи. Конкретные примеры ответов прямо в промпте. Эмоция `opinionated` — у Миры есть своё мнение.
- **Поле `gender` теперь попадает в системный промпт.** Был баг — `load_persona()` собирала промпт по полям, а `gender` (добавленное позже) не упоминалось. Мира иногда соскальзывала в мужской род.
- **Баг: `cmd_whoami`/`cmd_users`/`cmd_files`/`cmd_clear` падали при вызове из inline-кнопки.** `update.message` в callback-контексте равен `None` → `AttributeError: 'NoneType' object has no attribute 'reply_text'`. Добавлен хелпер `_reply(update, ...)` который работает в обоих контекстах, все 58 вызовов `update.message.reply_text(` заменены.
- **`/release` стабилизирован.** Работает с любой ветки (раньше требовал mira-dev), делает `fetch` перед мержем, авто-stash для локальных правок `persona.json`, использует `GITHUB_TOKEN` из env для HTTPS push без интерактивного ввода. Реальная ошибка показывается в Telegram, не "смотри лог".
- **Новый инструмент `git_log`.** Мира может смотреть собственную историю изменений (хеш, дата, сообщение). Раньше пыталась читать GitHub через `web_search` — приватный репо не отдавался.
- **SSH-доступ к VPS из Claude Code.** Ключ добавлен в `~/.ssh/authorized_keys`. Логи и состояние сервера читаются напрямую.

---

## Что изменилось в v3.9

Большой день — пользователи, память, Google Drive, веб-интерфейс:

- **Система управления пользователями v1.0.** Новые статусы: `rejected` (отклонён, помнит историю), `blacklisted` (ЧС, молчание + уведомление раз в сутки). Новые пользователи → `guest` (не `regular`). `/users` → список с inline-кнопками → карточка: одобрить/отклонить/ЧС/детский режим/удалить с подтверждением. `/blacklist`, `/evolution_count`. При одобрении пользователь получает приветствие с описанием возможностей, при отказе — вежливое сообщение.
- **Счётчик эволюций.** `memory/evolution_counter.json` — total/success/failed. `increment_evolution()` вызывается в `evolve()` при успехе и провале. `/evolution_count` показывает статистику.
- **Улучшенная долгосрочная память.** SUMMARY_TRIGGER: 30→20, KEEP_RECENT: 15→10, токенов на резюме: 500→900. Резюме структурировано: КТО / ПРОЕКТЫ / ПРЕДПОЧТЕНИЯ / КЛЮЧЕВЫЕ ФАКТЫ / ТЕКУЩЕЕ. Профиль обновляется на 16 последних сообщениях (было 10), порог снижен до 6.
- **Google Drive: триггерная синхронизация файлов.** `sync_output_to_drive()` — после `write_file`/`excel_write` файлы из `output/` летят на Drive в фоне. `sync_inbox_from_drive()` — перед `list_files`/`read_file` из inbox тянет новые файлы с Drive. Пользователь кладёт файл на Drive → Мира видит сразу.
- **Логи на Google Drive.** cron 4:00 UTC: `rclone copy logs/ gdrive:Mira/logs`.
- **Веб-интерфейс: Telegram Login** (без статического токена). Callback mode — сессия устанавливается в основном окне без попапа. Файлы: загрузка через 📎, скачивание из панели "Файлы". Команды: Профиль / Файлы / Очистить / Сброс профиля.
- **Редизайн веб-интерфейса** в стиле логотипа MIRA_BOT: тёмно-синий фон, градиент синий→фиолетовый, SVG-компас.
- **Исправлен баг** с tool-сообщениями в сессии (400 при fallback на DeepSeek).
- **notify_owner()** теперь реально отправляет в Telegram с inline-кнопками.
...
- **APK для Android** — нативное приложение для обхода блокировок Telegram, с прямым WebSocket-соединением к VPS. Не PWA.
