...
### ЭТАП 8 — Самосознание Миры  ← СЛЕДУЮЩИЙ
...
- [ ] **`list_self()`** — перечислить структуру проекта (безопасные папки: `agents/`, `tools/`, `profiles/`, корень).
- [ ] **`read_self(path)`** — прочитать файл проекта по whitelist: `agent.py`, `conclave.py`, `router.py`, `providers.py`, `tools/*.py`, `agents/*.json`, `profiles/*.json`, `persona.json`, `PRINCIPLES.md`. Запрещено: `.env`, `memory/`, `workspace/`.
- [ ] Добавить `list_self`, `read_self` в `TOOL_SCHEMAS` и `execute_tool`.
- [ ] Добавить в `dev.json` и `default.json` в `allowed_tools`.
- [ ] Обновить `persona.json` — Мира знает об этих инструментах и умеет ими пользоваться: читает свой код когда нужно понять поведение, проверяет принципы при сомнениях, изучает специалистов перед делегированием.
...
### ЭТАП 9 — Приватность и шифрование
...
- [ ] Пакет `cryptography` (Fernet — симметричное шифрование).
- [ ] `MEMORY_ENCRYPTION_KEY` в `.env` — генерируется один раз (`Fernet.generate_key()`).
- [ ] Шифровать при записи, расшифровывать при чтении: `memory/sessions/*.json`, `memory/*.json` (профили).
- [ ] `memory/decisions.log` — не шифруем (технический, без личных данных).
- [ ] Бэкап на Google Drive уже зашифрован на уровне rclone crypt — проверить/настроить.

#### 9.2 — Разделение логов
- [ ] Технические логи: `logs/agent.log` — уже реализовано (ротация, 3 дня).
- [ ] Чат-транскрипты: сейчас хранятся в `memory/sessions/` как JSON. При необходимости добавить отдельный читаемый лог разговоров в `logs/chats/{user_id}/`.
- [ ] `decisions.log` — уже отдельно в `memory/`.
- [ ] **Вывод:** разделение уже достаточное. Чат-транскрипты добавим только если возникнет конкретная потребность.
...
#### 10.3 — Мультичат (опционально)
- [ ] **Решение:** мульти-чат не нужен если есть долгая память (Этап 5). Один разговор с умной памятью эквивалентен специализированным приложениям.
...
- [ ] Мира сама обновляет профиль пользователя при появлении новой инфы (имя, предпочтения, контекст).
- [ ] Ротация sessions: когда история превышает лимит — сжать старое в summary, сохранить в профиле.
- [ ] Summary пишется в `memory/{user_id}.json` в поле `conversation_summary`.
...
[ ] ЭТАП 8   — Самосознание: list_self, read_self, обновление персоны   ← СЛЕДУЮЩИЙ
[ ] ЭТАП 9   — Приватность: шифрование памяти (Fernet), безопасность .env
[ ] ЭТАП 10  — Веб-интерфейс: FastAPI, WebSocket, Nginx, HTTPS
[ ] ЭТАП 5   — Долгая память: суммаризация, обновление профиля, шаблоны
[ ] ЭТАП 6   — Тесты
```
