...
| 🎨 **Aurora-веб** | Браузерный клиент переведён на Aurora (полночь+золото, 3 шрифта) + паритет с мобайлом: справочные команды и список пользователей раскрываются в боковом меню (не в чате), кнопка прыжка вниз. |
