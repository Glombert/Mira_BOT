...
[![Version](https://img.shields.io/badge/version-1.0-brightgreen?style=for-the-badge)](https://github.com/Glombert/Mira_BOT)
...
- **Понимать себя** — читает собственный код и конфиги, обновляет персону через `write_persona`
...
| Vision | Claude Sonnet 4.6 (фото в чате) | ✓ |
| Excel | openpyxl | ✓ |
| Поиск | Perplexity sonar-pro → DuckDuckGo (ddgs) | ✓ |
| Шифрование | Fernet (memory/) | ✓ |
| Память | JSON + шифрование | ✓ |
| Логи | TimedRotatingFileHandler, 3 дня | ✓ |
| Бэкап | rclone → Google Drive, cron | ✓ |
...
[ ] v1.1      — Долгая память: суммаризация, шаблоны задач
[ ] v1.2      — Веб-интерфейс: FastAPI, WebSocket, Nginx
[ ] v1.x      — Тесты, мониторинг, изоляция run_python
```
