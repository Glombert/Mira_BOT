...
| 👤 **Экран «Профиль» (Lite)** | WS-команда `profile_data` отдаёт структурированный профиль (имя/роль/часовой пояс/Drive/число фактов памяти/число бесед/что Мира знает о тебе). Клиенты рисуют отдельный экран: hero «наша связь», личные данные с inline-правкой часового пояса, статус Drive, «Забыть меня» с подтверждением вводом «забудь». Пункт «Профиль» в меню теперь открывает экран, а не info-панель. Веб + мобайл. |
