...
const FIXTURES: Record<string, unknown> = {
  ...
  ws_message_rate_limited: {
    type: 'message',
    content: 'Эй, помедленнее — я ещё не успеваю отвечать. Подожди 24 сек и продолжим.',
  },
...
export class MiraClient {
  ...
  // Прогрессия 1s,2s,4s,...,30s,30s,... → 20 попыток ≈ 8 минут.
  // После — перестаём пытаться, чтобы не молотить сеть в фоне до бесконечности.
  ...
  async loadSession(): Promise<string | null> {
    ...
    // с пустым ?session= (this.session = null, хотя в keychain токен есть).
    // Это даёт серверу пустую сессию → WS закрывается 4001 →
    // клиент видит «Сессия истекла».
    if (this._sessionStorage) {
  ...
  uploadFile(file: { uri: string; name: string; type: string }): Promise<UploadResult> {
    if (this.mock) {
      return new Promise((resolve) => {
        setTimeout(() => resolve(FIXTURES.upload_success as UploadResult), 1500);
      });
    }
  ...
  /** Принудительное немедленное переподключение (кнопка / возврат приложения
   * на экран). Отменяет отложенный реконнект, сбрасывает backoff и подключается
   * сразу. Слушатели (on/onTech) переживают реконнект. */
  ...
  private _scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      this._emit({
        type: 'error',
        content: 'Не удалось подключиться к серверу. Проверь интернет и потяни вниз для повторной попытки.',
      });
      return;
    }
  ...
  sendMessage(text: string, attachments?: string | string[], mode?: 'chat' | 'tech'): void {
    const payload: ClientMessage & { attachments?: string[]; mode?: 'chat' | 'tech' } = { content: text };
    if (attachments) {
  ...
  private _emit(msg: ServerMessage): void {
    ...
    // ответы Миры в tech-режиме. Не идёт в общий чат и не разбирается awaiter'ами.
